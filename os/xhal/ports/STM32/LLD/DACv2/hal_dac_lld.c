...
msg_t dac_lld_start_conversion(DACDriver *dacp) {
  ...
  dacddma = (((cfg->mcr & ~dacp->params->regmask) >> dacp->params->regshift) &
             DAC_MCR_DMADOUBLE1) != 0U;
  ...
  cr |= (DAC_CR_DMAEN1 | (dacp->grpp->trigger << DAC_CR_TSEL1_Pos) |
         DAC_CR_TEN1 | cfg->cr) << dacp->params->regshift;
...
void dac_lld_stop_conversion(DACDriver *dacp) {
  ...
  mcr |= ((((const DACConfig *)dacp->config)->mcr & dacp->params->regmask) <<
          dacp->params->regshift);
  ...
  cr &= dacp->params->regmask;
  cr |= (((const DACConfig *)dacp->config)->cr & CONFIG_SINGLE_MASK) <<
        dacp->params->regshift;
