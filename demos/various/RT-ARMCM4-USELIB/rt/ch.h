...
#define CH_CFG_OS_INSTANCE_INIT_HOOK(oip) do { } while (false)
#define CH_CFG_THREAD_EXTRA_FIELDS 
#define CH_CFG_THREAD_INIT_HOOK(tp) do { } while (false)
#define CH_CFG_THREAD_EXIT_HOOK(tp) do { } while (false)
#define CH_CFG_CONTEXT_SWITCH_HOOK(ntp,otp) do { } while (false)
...
#define CH_CFG_SYSTEM_HALT_HOOK(reason) do { } while (false)
#define CH_CFG_TRACE_HOOK(tep) do { } while (false)
#define CH_CFG_RUNTIME_FAULTS_HOOK(mask) do { } while (false)
...
typedef port_stkline_t stkline_t;
...
#define SFT_IS_VALID_DATA_POINTER(p) (((p) != NULL) && (((uintptr_t)(p) & (PORT_NATURAL_ALIGN - 1U)) == 0U))
#define chSftAssert(l,c,r) do { if (((l) <= CH_CFG_HARDENING_LEVEL) || (((l) <= 2) && (CH_DBG_ENABLE_ASSERTS != FALSE))) { if (unlikely(!(c))) { CH_CFG_SAFETY_CHECK_HOOK(l, __func__); } } } while (false)
...
#define ch_queue_for_each(pos,head) for (pos = (head)->next; pos != (head); pos = pos->next)
#define ch_queue_for_each_reverse(pos,head) for (pos = (head)->prev; pos != (head); pos = pos->prev)
#define ch_queue_get_owner(ptr,type,member) __CH_OWNEROF(ptr, type, member)
#define ch_queue_first_owner(head,type,member) __CH_OWNEROF((head)->next, type, member)
#define ch_queue_last_owner(head,type,member) __CH_OWNEROF((head)->prev, type, member)
...
#define CHMEM_H 
#define CH_MEM_GLOBAL_BSS 
#define CH_MEM_LOCAL_BSS(c) CH_MEM_GLOBAL_BSS
#define CH_MEM_PRIVATE_BSS(c) CH_MEM_LOCAL_BSS(c)
#define CH_MEM_GLOBAL_COHERENT_BSS 
#define CH_MEM_LOCAL_COHERENT_BSS(c) CH_MEM_GLOBAL_COHERENT_BSS
#define CH_MEM_PRIVATE_COHERENT_BSS(c) CH_MEM_LOCAL_COHERENT_BSS(c)
#define CHSYS_H 
      #define currcore (&ch0)
...
#define CH_FLAGS_USER_MASK (tmode_t)127U
#define CH_FLAGS_TERMINATE (tmode_t)128U
...
#define THD_STACK_SIZE(n) MEM_ALIGN_NEXT(PORT_WA_SIZE(n), PORT_STACK_ALIGN)
#define THD_STACK(s,n) CC_ALIGN_DATA(PORT_WORKING_AREA_ALIGN) stkline_t s[THD_STACK_SIZE(n) / sizeof (stkline_t)]
#define THD_STACK_BASE(s) ((stkline_t *)(s))
#define THD_STACK_END(s) (THD_STACK_BASE(s) + (sizeof (s) / sizeof (stkline_t)))
#define THD_WORKING_AREA_SIZE(n) (MEM_ALIGN_NEXT(PORT_WA_SIZE(n), PORT_STACK_ALIGN) + MEM_ALIGN_NEXT(sizeof (thread_t), PORT_STACK_ALIGN))
#define THD_WORKING_AREA(s,n) CC_ALIGN_DATA(PORT_WORKING_AREA_ALIGN) stkline_t s[THD_WORKING_AREA_SIZE(n) / sizeof (stkline_t)]
#define THD_WORKING_AREA_BASE(s) ((stkline_t *)(s))
#define THD_WORKING_AREA_END(s) (THD_WORKING_AREA_BASE(s) + (sizeof (s) / sizeof (stkline_t)))
